...
export default class IntegrationList extends ComponentWithModal {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      ...
      nextProps.Task.detail.integrations.integration.id !=
        this.props.Task.detail.integrations.integration.id
    ) {
  ...
  render() {
    ...
    return (
      <div className="form-wrapper">
        ...
        Task.detail.integrations.isRetrieving ||
        github.repos.isFetching ||
        github.issues.isFetching ||
        slack.isRetrieving
          ? <Progress />
          : <div>
              {provider == SOCIAL_PROVIDERS.slack
                ? <div>
                    <div className="card">
                      {slack.isConnected
                        ? <div>
                            ...
                            Team: <strong>{integration.team_name}</strong>
                            <br />
                    ...
                    {!slack.isConnected
                      ? <a
                          href={
                            SOCIAL_LOGIN_URLS.slack +
                            `?action=connect&task=${task.id}&next=${window
                              .location.protocol}//${window.location
              ...
              {provider == SOCIAL_PROVIDERS.github
                ? <div>
                    <div className="card">
                      Connect your task to a GitHub repository or issue to show
                      GitHub activity (e.g comments, pull requests, push events)
                      in your task activity stream.
                    </div>
                    {!github.isConnected
                      ? <a
                          href={
                            SOCIAL_LOGIN_URLS.github +
                            `?action=connect&task=${task.id}&next=${window
                              .location.protocol}//${window.location
                    ...
                    <div className="form-group">
                      ...
                      {event_choices.map(event => {
                        if (
                          this.state.integration_type ==
                            INTEGRATION_TYPE_ISSUE &&
                          event.id != INTEGRATION_EVENT_ISSUE_COMMENT
                        ) {
                          return null;
                        }
                        return (
                          <div key={event.id} className="checkbox">
                            <label className="control-label">
                              <input
                                ...
                                checked={
                                  this.state.events.indexOf(event.id) > -1
                                }
                              />
                              {event.name}
                            </label>
